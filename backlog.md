...
- [vat-46x] Make the "/vat" skill default to using the vat binary if it's installed. If it's not installed, it just does what it does today
...
- [vat-mzd] [agent-ready] Wire `vat sync` to append the `(see ./items/<id>.md)` pointer suffix — implement SYNC-PTR-001..003 (see ./items/vat-mzd.md)
---
